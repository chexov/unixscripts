...
class Client(Connection):
    ...
    def auth(self,username,passwd,resource):
        ...
        if auth_ret_query.getTag('token'):
            ...
            hash = hashlib.new('sha', hashlib.new('sha', passwd).hexdigest()+token).hexdigest()
            for foo in xrange(int(seq)): hash = hashlib.new('sha', hash).hexdigest()
            ...
            digest.insertData(hashlib.new('sha',
...
class Component(Connection):
    ...
    def auth(self,secret):
        ...
        self.send( u"<handshake id='1'>%s</handshake>"
                   % hashlib.new('sha', self.getIncomingID() + secret ).hexdigest()
                  )
